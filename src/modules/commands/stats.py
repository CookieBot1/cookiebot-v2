...
@bot.command(aliases = ["mystats", "info"])
async def stats(ctx, userID = '0'):
    try:
        ...
        stats_embed.add_field(name = "Total Count", value = str(userCounter) + " Numbers", inline = True)
        stats_embed.add_field(name = "Counter Fails", value = str(userFailCounter) + " Fails", inline = True)
