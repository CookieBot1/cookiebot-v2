...
async def countboard(ctx: commands.Context):
    ...
    if not guild_users:
        return await ctx.reply("No users have counted here!", delete_after=7)
...
async def page_buttons(interaction: discord.Interaction, view: discord.ui.View | None):
    ...
    guild_users: dict = guild_data.get("users", {})
    if not guild_users:
        return await interaction.response.send_message("No users have counted here!", ephemeral=True)
...
async def build_embed(guild_users: dict, author_id: str, page_num: int = 0) -> discord.Embed:
    simplified_users: list[SimpleUser] = [
        SimpleUser(uid, data["Counter"]) for uid, data in guild_users.items()
    ]
...
async def build_view_page(all_users: list[SimpleUser], page_num: int = 0) -> discord.Embed:
    ...
    # Build the embed.
    embed = discord.Embed(title="Count Leaderboard", color=0x7688D4)
