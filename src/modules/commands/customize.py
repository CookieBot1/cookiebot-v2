...
async def customize(ctx, category = 'general'):
    ...
    try:        
        ...
        if category == "profile":
            ...
            try:
                ...
                return await ctx.send("Customization timed out. Please try again.")

            ## start the bio editing prompt
            ...
            elif msg.content.lower() not in {'no', 'n'}:
                m = await ctx.send("Please respond with `yes` or `no` next time. Skipping bio edit.")
                cleanup.append(m)

            ## after bio prompt, start color prompt
            ...
            if msg.content.lower() in ['yes', 'y']:
                ...
                if chosen_color is None:
                    return await ctx.send("Invalid color choice. Please try the command again.")
                else:
                ...
                m = await ctx.send("Please respond with `yes` or `no` next time. Skipping color change.")
                cleanup.append(m)

            ## after color prompt, start show stats on profile prompt
            ...
            if msg.content.lower() in ['yes', 'y']:
                ...
                "Shortcuts: `all`, `none`, `reset`, `cancel`"
                )
                cleanup.append(m)
                ...
                if raw in {"cancel", "stop", "exit"}:
                    m = await ctx.send("❌ Cancelled. No changes made.")
                    cleanup.append(m)
                    return
